...
def index(request):
    ...
    date_list = [passed_date + datetime.timedelta(days=x)
                                                  for x in range((current_date-passed_date).days)]
    months = get_months_between_dates(passed_date, current_date)
    print(months)
    total_sales = Order.objects.filter(ordered=True, being_delivered=True).aggregate(
        Sum('payment__amount'))['payment__amount__sum']
